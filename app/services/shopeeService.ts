...
export async function getBookingList(shopId: number, options: BookingListOptions = {}) {
  try {
    ...
    if (response.error) {
      console.error(`Error saat mengambil daftar booking: ${JSON.stringify(response)}`);
      return {
        success: false,
        error: response.error,
    ...
    console.info(`Berhasil mengambil daftar booking untuk toko ${shopId}`);
    return {
      success: true,
      data: response.response,
      request_id: response.request_id
...
export async function getBookingDetail(
  ...
  try {
    ...
    if (typeof bookingSn === 'string') {
      ...
      if (!bookingSn || bookingSn.length === 0 || bookingSn.some(sn => !sn.trim())) {
        return {
          success: false,
          error: "invalid_input",
          message: 'Nomor booking diperlukan dan tidak boleh kosong'
        };
      }
      bookingSns = bookingSn.map(sn => sn.trim());
    }

    // Default response optional fields lengkap dari dokumentasi Shopee API
    ...
    // Gunakan responseOptionalFields yang diberikan, atau default lengkap
    const fieldsToUse = responseOptionalFields || defaultOptionalFields;

    const accessToken = await getValidAccessToken(shopId);
    const response = await shopeeApi.getBookingDetail(
      shopId, 
      accessToken, 
      bookingSns,
      fieldsToUse
    ...
    if (response.error) {
      console.error(`Error saat mengambil detail booking: ${JSON.stringify(response)}`);
      return {
        success: false,
        error: response.error,
    ...
    const bookingInfo = bookingSns.length === 1 ? bookingSns[0] : `${bookingSns.length} booking`;
    console.info(`Berhasil mengambil detail booking ${bookingInfo} untuk toko ${shopId}`);
    return {
      success: true,
      data: response.response,
      request_id: response.request_id
...
export async function getBookingShippingParameter(
  ...
  try {
    ...
    const accessToken = await getValidAccessToken(shopId);
    const response = await shopeeApi.getBookingShippingParameter(
      shopId, 
      accessToken, 
      bookingSn.trim()
    );
    ...
    if (response.error) {
      console.error(`Error saat mengambil parameter pengiriman booking: ${JSON.stringify(response)}`);
      return {
        success: false,
        error: response.error,
    ...
    console.info(`Berhasil mengambil parameter pengiriman booking ${bookingSn} untuk toko ${shopId}`);
    return {
      success: true,
      data: response.response,
      request_id: response.request_id
  ...
  try {
    ...
    if (!['pickup', 'dropoff'].includes(shippingMethod)) {
      return {
        success: false,
        error: "invalid_input",
        message: 'Metode pengiriman harus pickup atau dropoff'
      };
    }

    const accessToken = await getValidAccessToken(shopId);
    
    // Jika shippingData tidak diberikan, ambil dari parameter pengiriman
    let pickupData = null;
    ...
    if (shippingData) {
      ...
      // Ambil parameter pengiriman otomatis
      ...
      if (shippingMethod === 'pickup') {
        if (shippingParams.response.pickup) {
          ...
          return {
            success: false,
            error: "pickup_not_available",
            message: `Metode pickup tidak tersedia untuk booking ${bookingSn}`,
            request_id: shippingParams.request_id || ''
          };
        }
      } else {
        if (shippingParams.response.dropoff) {
          ...
          return {
            success: false,
            error: "dropoff_not_available",
            message: `Metode dropoff tidak tersedia untuk booking ${bookingSn}`,
            request_id: shippingParams.request_id || ''
          };
        }
      }
    ...
    const response = await shopeeApi.shipBooking(
      shopId, 
      accessToken, 
      bookingSn.trim(),
      pickupData,
    ...
    if (response.success) {
      console.info(`Booking ${bookingSn} berhasil dikirim dengan metode ${shippingMethod}: ${JSON.stringify(response)}`);
    } else {
      console.error(`Terjadi kesalahan saat mengirim booking ${bookingSn}: ${JSON.stringify(response)}`);
    }
...
export async function getBookingTrackingNumber(
  ...
  try {
    ...
    const accessToken = await getValidAccessToken(shopId);
    const response = await shopeeApi.getBookingTrackingNumber(
      shopId, 
      accessToken, 
      bookingSn.trim(),
      packageNumber
    ...
    if (response.error) {
      console.error(`Error saat mengambil tracking number booking: ${JSON.stringify(response)}`);
      return {
        success: false,
        error: response.error,
    ...
    console.info(`Berhasil mengambil tracking number untuk booking ${bookingSn} di toko ${shopId}`);
    return {
      success: true,
      data: response.response,
      request_id: response.request_id
  ...
  try {
    ...
    if (!bookingList || bookingList.length === 0) {
      return {
        success: false,
        error: "invalid_input",
        message: 'Daftar booking tidak boleh kosong'
      };
    }
    ...
    if (bookingList.length > 50) {
      return {
        success: false,
        error: "invalid_input",
        message: 'Jumlah booking tidak boleh lebih dari 50'
      };
    }
    ...
    if (!validDocumentTypes.includes(documentType)) {
      return {
        success: false,
        error: "invalid_input",
        message: `Document type tidak valid. Harus salah satu dari: ${validDocumentTypes.join(', ')}`
      };
    }

    const accessToken = await getValidAccessToken(shopId);
    const response = await shopeeApi.createBookingShippingDocument(
      shopId,
      accessToken,
      bookingList,
      documentType
    ...
    if (response.error) {
      console.error(`Error saat membuat dokumen pengiriman booking: ${JSON.stringify(response)}`);
      return {
        success: false,
        error: response.error,
...
export async function downloadBookingShippingDocument(
  ...
  return retryOperation(async () => {
    ...
    console.log('Download booking document request:', {
      shopId,
      bookingList,
      accessToken: accessToken ? 'exists' : 'missing'
    });
    ...
    if (!shopId || !bookingList || bookingList.length === 0) {
      ...
      return {
        error: "invalid_parameters",
        message: "Parameter shopId dan bookingList harus diisi"
      };
    }

    if (bookingList.length > 50) {
      console.error('Too many bookings');
      return {
        error: "invalid_parameters",
    ...
    for (let i = 0; i < bookingList.length; i++) {
      const booking = bookingList[i];
      if (!booking.booking_sn || booking.booking_sn.trim().length === 0) {
        console.error(`Missing booking_sn at index ${i}`);
        return {
          error: "invalid_parameters",
          message: `booking_sn tidak boleh kosong pada index ${i}`
        };
    ...
    const response = await shopeeApi.downloadBookingShippingDocument(
      shopId, 
      accessToken, 
    ...
    if (response.error) {
      ...
      return {
        error: response.error,
        message: response.message || "Gagal mengunduh dokumen booking dari Shopee API",
        request_id: response.request_id || ''
      };
    }
